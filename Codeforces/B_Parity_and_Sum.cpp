...
void solve() {
    ...
    //  make all even -> odd : 
    //  choose max odd : Sort even : set each e[i] += maxodd
    //  update maxodd
    int maxodd = MAX(o);
    int ans = e.size();
    forr(i, e.size()) {
        if (e[i] < maxodd)maxodd += e[i];
        else {
            // when maxodd < e[i] : then choose the MAX EVEN
            // then 1 Operation in updating maxodd += MAX EVEN
            // we have me the maximum ODD now :
            // thats the reason we do ans++ once
            ans++;
            break;
        }
    }
